...
from mmscene.engine.hooks import SyncNormHook


class TestSyncNormHook(TestCase):

    @patch(
        'mmscene.engine.hooks.sync_norm_hook.get_dist_info', return_value=(0, 1))
    ...
    @patch(
        'mmscene.engine.hooks.sync_norm_hook.get_dist_info', return_value=(0, 2))
